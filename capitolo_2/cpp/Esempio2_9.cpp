...
// --------------------------
// Equazione differenziale - Carrello Molla
// --------------------------
...
// --------------------------
// Equazione differenziale - Modello semplificato
// --------------------------
std::vector<double> f_sistemaCarrelloMollaSemplificato(double t, const std::vector<double> &x, const Params &p, double &dummy)
{
    double u = p.Fm;
    double Fe = 0.95 * p.kt0 * x[0];
    std::vector<double> dx(2);
    dx[0] = x[1];
    dx[1] = (u - Fe) / p.M;
    return dx;
...
int main()
{
    ...
    auto y_teor = rungeKutta4(f_sistemaCarrelloMollaSemplificato, {s0, s0dot}, t0, tf, dt, Params{M, Fm, kt0, alpha, t0}, dummy);
    std::vector<double> s_teor, sdot_teor, et_teor;
    for (auto &v : y_teor)
    {
        s_teor.push_back(v[0]);
        sdot_teor.push_back(v[1]);
    }
    for (int i = 0; i <= N; ++i)
    {
        double Ue = (kt0 * 0.95 * s_teor[i] * s_teor[i]) / 2;
        double Up = (M * sdot_teor[i] * sdot_teor[i]) / 2;
        et_teor.push_back(Ue + Up);
    }
    ...
    fprintf(gp, "set ylabel 'Spostamento'\n");
    fprintf(gp, "set grid\n");
    fprintf(gp, "set key top right\n"); // Posizione della legenda (opzionale)

    fprintf(gp, "plot '-' with lines title 's(t)', '-' with lines title 's_{teor}(t)'\n");
    ...
    fprintf(gp, "set title 'Comportamento del sistema carrello-molla'\n");
    fprintf(gp, "set xlabel 'Tempo (t)'\n");
    fprintf(gp, "set ylabel 'Velocità'\n");
    fprintf(gp, "set grid\n");
    fprintf(gp, "set key top right\n"); // Posizione della legenda (opzionale)

    fprintf(gp, "plot '-' with lines title 'sdot(t)', '-' with lines title 'sdot_{teor}(t)'\n");
    ...
    // FIGURA: Comportamento del sistema carrello-molla
    // ==========================================
    fprintf(gp, "set terminal qt 3 title 'Figura'\n"); // Apre la finestra Qt 1
    fprintf(gp, "set title 'Comportamento del sistema carrello-molla'\n");
    fprintf(gp, "set xlabel 'Tempo (t)'\n");
    fprintf(gp, "set ylabel 'Energia totale'\n");
    fprintf(gp, "set grid\n");
    fprintf(gp, "set key top right\n"); // Posizione della legenda (opzionale)

    fprintf(gp, "plot '-' with lines title 'E(t)', '-' with lines title 'E_{teor}(t)'\n");
