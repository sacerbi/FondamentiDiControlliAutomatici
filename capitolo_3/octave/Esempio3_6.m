function Esempio3_6()
  ...
  u = ones(size(t)); % Ipotizzo forza esterna agente pari a zero
  ...
  xf = 1/(M*(s1-s2)) .* [(e.^(s1.*t)-1)./s1 - (e.^(s2.*t)-1)./s2;
                          e.^(s1.*t)-e.^(s2.*t)];
  X = [xl(1,:)+xf(1,:); xl(2,:)+xf(2,:)];
  % Confronto la soluzione matematica con la soluzione del risolutore
  comparazione(A,B,C,D,u,t,x0,X(1,:),X(2,:));
  ...
  xf2 = 1/M2 .* [1/(d*d) .* (1 + d/w .* e.^(s.*t) .* sin(w.*t -g));
                  1/w .* e.^(s.*t) .* sin(w.*t)];
  X2 = [xl2(1,:)+xf2(1,:); xl2(2,:)+xf2(2,:)];
  % Confronto la soluzione matematica con la soluzione del risolutore
  comparazione(A2,B2,C2,D2,u,t,x0,X2(1,:),X2(2,:));
  ...
  xf3 = 1/M .* [1/(s0*s0) - (e.^(s0.*t))./(s0*s0) + (t.*e.^(s0.*t))./s0; t.*e.^(s0.*t)];
  X3 = [xl3(1,:)+xf3(1,:); xl3(2,:)+xf3(2,:)];
  % Confronto la soluzione matematica con la soluzione del risolutore
  comparazione(A3,B3,C3,D3,u,t,x0,X3(1,:),X3(2,:));
endfunction
